...
function OrderExecutabilityOne({ widgetId, data, loading, isMockData }) {
  const [selectedStatus, setSelectedStatus] = useState(null);
  const [modalOpen, setModalOpen] = useState(false);

  console.log(`[OrderExecutabilityOne] Рендер виджета ${widgetId}, данные:`, data ? 'есть' : 'нет', 'тестовые:', isMockData);
  ...
  const stats = [
    { label: 'Оплаченные', value: data.paid, status: 'paid', color: config.colors.paid },
    { label: 'Неоплаченные', value: data.unpaid, status: 'unpaid', color: config.colors.unpaid },
    { label: 'Частичная оплата', value: data.processing, status: 'processing', color: config.colors.processing }
  ];
...
export default OrderExecutabilityOne;