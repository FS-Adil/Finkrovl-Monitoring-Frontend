...
function OrderExecutabilityTwo({ widgetId, data, loading, isMockData }) {
  const [selectedStatus, setSelectedStatus] = useState(null);
  const [modalOpen, setModalOpen] = useState(false);

  console.log(`[OrderExecutabilityTwo] Рендер виджета ${widgetId}, данные:`, data ? 'есть' : 'нет', 'тестовые:', isMockData);
  ...
  const stats = [
    { label: 'Нет проведенных производств', value: data.paid, status: 'paid', color: config.colors.paid },
    { label: 'Есть частичное производство', value: data.unpaid, status: 'unpaid', color: config.colors.unpaid },
  ];
...
export default OrderExecutabilityTwo;